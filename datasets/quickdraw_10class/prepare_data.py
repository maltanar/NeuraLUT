...
def download(nums="10"):
    ...
    for c in classes:
        ...
        target_path = os.path.join(raw_root, c + ".npy")
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Download Quick, Draw! data from Google and then "
            "dump the raw data into cache."
        ),
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.2,
        help="Specify the val fold ratio.",
    )
    parser.add_argument(
        "--max_samples_category",
        "-msc",
        type=int,
        default=5000,
    ...
    parser.add_argument(
        ...
        choices=[0, 1],
        default=0,
        help="1 for download data, 0 for not.",
    )
    parser.add_argument(
        "--show_random_imgs",
        "-show",
        action="store_true",
        default=False,
        help="show some random images while generating the dataset.",
    )
    ...
    generate_dataset(
        vfold_ratio=args.valfold,
        max_samples_per_class=args.max_samples_category,
