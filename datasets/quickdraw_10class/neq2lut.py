...
from models import QuickDrawNeqModel, QuickDrawLutModel
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="Root directory containing QuickDraw train.npz/test.npz",
    )
    parser.add_argument(
        "--log-dir",
        type=str,
    ...
    # Fetch the selected QuickDraw split.
    dataset = {}
    dataset[config["dataset_split"]] = QD_Dataset(
        config["dataset_split"], root=config["data_root"]
    )
    test_loader = DataLoader(
        dataset[config["dataset_split"]], batch_size=config["batch_size"], shuffle=False
    )
    ...
    lut_model = QuickDrawLutModel(model_cfg)
    if options_cfg["cuda"]:
        lut_model.cuda()
